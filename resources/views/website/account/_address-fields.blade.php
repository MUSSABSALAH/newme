...
<div class="field">
  ...
  <input type="text" id="label-{{ $prefix }}" name="label" value="{{ old('label', $address?->label) }}" placeholder="{{ __('addresses.placeholders.label') }}" required>
  @error('label')<div class="err">{{ $message }}</div>@enderror
</div>

<div class="field">
  <label for="recipient_name-{{ $prefix }}">{{ __('addresses.fields.recipient_name') }}</label>
  <input type="text" id="recipient_name-{{ $prefix }}" name="recipient_name" value="{{ old('recipient_name', $address?->recipient_name ?? auth()->user()?->name) }}" required>
  @error('recipient_name')<div class="err">{{ $message }}</div>@enderror
</div>

<div class="field">
  <label for="phone-{{ $prefix }}">{{ __('addresses.fields.phone') }}</label>
  <input type="text" id="phone-{{ $prefix }}" name="phone" value="{{ old('phone', $address?->phone ?? auth()->user()?->phone) }}" required dir="ltr">
  @error('phone')<div class="err">{{ $message }}</div>@enderror
</div>

<div class="field">
  <label for="city-{{ $prefix }}">{{ __('addresses.fields.city') }}</label>
  <input type="text" id="city-{{ $prefix }}" name="city" value="{{ old('city', $address?->city) }}" required>
  @error('city')<div class="err">{{ $message }}</div>@enderror
</div>

<div class="field">
  <label for="district-{{ $prefix }}">{{ __('addresses.fields.district') }}</label>
  <input type="text" id="district-{{ $prefix }}" name="district" value="{{ old('district', $address?->district) }}" required>
  @error('district')<div class="err">{{ $message }}</div>@enderror
</div>

<div class="field">
  <label for="street-{{ $prefix }}">{{ __('addresses.fields.street') }}</label>
  <input type="text" id="street-{{ $prefix }}" name="street" value="{{ old('street', $address?->street) }}" required>
...
<div class="field">
  ...
  <input type="text" id="national_address-{{ $prefix }}" name="national_address" value="{{ old('national_address', $address?->national_address) }}" placeholder="{{ __('addresses.placeholders.national_address') }}" required dir="ltr" autocomplete="off">
